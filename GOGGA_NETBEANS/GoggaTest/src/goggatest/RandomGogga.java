...
//		int dir = abug.getDirection();
	...
	public static void main(String[] args) {
		...
		int xGridSize = 14;
		int yGridSize = 10;
		Gogga.setGridSize(xGridSize, yGridSize);

		Gogga bug_blue = new Gogga();
		Gogga bug_red = new Gogga();
		bug_red.setColor(Color.red);
		bug_blue.setColor(Color.blue);

		boolean go = false;
		while (!go) {
			go = randomStepBounded(bug_red, xGridSize, yGridSize);
			if (go == true) {
				System.out.println("Red lost");
				break;
			}
			go = randomStepBounded(bug_blue, xGridSize, yGridSize);
			if (go == true) {
		...
		turnOpp(bugRY);
		turnOpp(bugYR);
		movebug(bugRY, bugYR, 3);
		randomMovement(bugYR, 3);
		randomMovement(bugRY, 3);
